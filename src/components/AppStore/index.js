...
class AppStore extends Component {
  state = {isActiveId: tabsList[0].tabId, searchInputValue: ''}
  ...
  getFilteredApps = searchedApps => {
    ...
    const filteredApps = searchedApps.filter(
      eachITem => eachITem.category === isActiveId,
    )
    return filteredApps
  }
  searchInput = event => {
  ...
  getSearchResult = () => {
    ...
    const searchResult = appsList.filter(eachApp =>
      eachApp.appName.toLowerCase().includes(searchInputValue.toLowerCase()),
    )
        ...
        <ul className="tabs-container">
          {tabsList.map(eachTab => (
            <TabItem
              ...
              isActive={eachTab.tabId === isActiveId}
            />
          ))}
        </ul>
